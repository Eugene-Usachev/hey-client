...
export const SignUpForm: FC<SignUpFormProps> = memo<SignUpFormProps>(({setIsLoginWindowOpen, login, onChangeLogin, onChangePassword, inputDict,
	...
	const SignUpCallBack = useCallback(() => {
		...
		return false;
	}, [login, name, surname, email, password, isLoginError, isEmailError]);

	return (
		<div className={styles.signUpBlock}>
			<h2>{dict.CreateNewAccount}</h2>
			<Input type={"default"} dict={inputDict} error={isLoginError ? dict.LoginIsBusy : ""} onChangeValue={(currentLogin) => {setIsLoginError(false); onChangeLogin(currentLogin)}} blockStyle={{width: '418px'}} onEnter={SignUpCallBack} placeholder={dict.EnterYourLogin} startValue={login} maxLength={32} minLength={1} style={{marginBottom: '5px', width: '400px'}} checkSpace={true}/>
			...
			<Input type={"default"} dict={inputDict} error={isEmailError ? dict.EmailIsBusy : ""} onChangeValue={(currentEmail) => {setIsEmailError(false); onChangeEmail(currentEmail)}} blockStyle={{width: '418px'}} onEnter={SignUpCallBack} placeholder={dict.EnterYourEmail} startValue={email} minLength={1} maxLength={32} style={{marginBottom: '5px', width: '400px'}} checkSpace={true}/>
